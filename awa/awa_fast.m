...
X = X';
[cof,sco]=princomp(X);
X_tr=sco(1:length(X_tr),1:100);
X_te = sco(length(X_tr)+1:length(X),1:100);
...
while(t<=max_iter)
    ...
    for i = 1:test_num
        ...
        for j=1:classes_num				
            if(n(i,j))
                c=c+n(i,j)*S_te_pro(j,:)'*X_te1(i,:);
                a=a+n(i,j)*S_te_pro(j,:)'*S_te_pro(j,:);
